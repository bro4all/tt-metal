...
if [[ -z "$TT_METAL_SLOW_DISPATCH_MODE" ]] ; then
    ...
    timeout 60 ./build/test/tt_metal/test_clean_init || { echo "Error: second run timed out, clean init (FD-on-Tensix) failed."; exit 1; }
    ...
    if [[ "$ARCH_NAME" == "wormhole_b0" ]]; then
        ...
        echo "Second run, expect clean init"
        timeout 60 env ./build/test/tt_metal/test_clean_init || { echo "Error: second run timed out, clean init (FD-on-Eth) failed."; exit 1; }
