...
def test_slice_my_case(device):
    ...
    # Convert slice bounds to TT tensors
    start_tt = ttnn.from_torch(start)
    end_tt = ttnn.from_torch(end)

    # Perform slicing on TT device
    sliced_tt = ttnn.slice(x_tt, start_tt, end_tt)
