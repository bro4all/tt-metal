...
def test_sdxl_base_group_norm(device, input_shape):
    ...
    print("sharded_mem_config: ", sharded_mem_config)
...
def test_sdxl_base_group_norm_negative_mask(device, input_shape):
    ...
    tt_input_tensor = ttnn.to_device(tt_input_tensor, device, memory_config=sharded_mem_config)
    print("tt_input_tensor: ", tt_input_tensor)
    ...
    tt_output_tensor = ttnn.from_device(tt_output_tensor)
    tt_output_tensor = ttnn.to_torch(tt_output_tensor)
    print("tt_output_tensor : ", tt_output_tensor)
...
@pytest.mark.parametrize("device_params", [{"l1_small_size": 0}], indirect=True)
@pytest.mark.parametrize(
    "N, C, H, W, num_groups, num_splits, grid_x",
    [
        (1, 640, 128, 128, 32, 2, 8),
        # (1, 960, 128, 128, 32, 1, 8),
    ],
)
def test_sdxl_base_group_norm_split(device, N, C, H, W, num_groups, num_splits, grid_x):
    torch.manual_seed(0)
    if device.core_grid.y == 7:
        pytest.skip()

    grid_size = ttnn.CoreGrid(y=8, x=grid_x)
    ...
    input_mask_tensor = ttnn.create_group_norm_input_mask(C_per_split, num_groups_per_split, grid_size.x)
    input_mask_tensor = ttnn.from_torch(
        input_mask_tensor,
        dtype=ttnn.DataType.BFLOAT8_B,
        layout=ttnn.TILE_LAYOUT,
    ...
    grid_coord = ttnn.CoreCoord(grid_size.x - 1, grid_size.y - 1)
    shard_grid = ttnn.CoreRangeSet({ttnn.CoreRange(ttnn.CoreCoord(0, 0), grid_coord)})
    shard_shape = N * H * W // grid_size.y, C_per_split // grid_size.x
    shard_spec = ttnn.ShardSpec(shard_grid, shard_shape, ttnn.ShardOrientation.ROW_MAJOR)
