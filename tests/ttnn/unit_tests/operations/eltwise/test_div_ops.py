...
def test_remainder_broadcast_case(device):
    x_torch = torch.rand([3, 512, 1024], dtype=torch.float32)
    y_torch = torch.rand([3, 1, 1], dtype=torch.float32)
    ...
    tt_out = ttnn.to_torch(z_tt_div)
    status = ttnn.pearson_correlation_coefficient(z_torch, tt_out) >= 0.999
    print(f"Pearson correlation coefficient: {ttnn.pearson_correlation_coefficient(z_torch, tt_out)}")
    assert status
