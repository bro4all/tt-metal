...
# fmt: off
@pytest.mark.parametrize("input_a,scalar,dtype", [
        ([13, 16, 42, 42], 0.125, torch.int32),
        ([], 1000, torch.int64),
        ([1, 2, 512, 1024], 4, torch.float32),
    ])
# fmt: on
def test_multiply_int32_with_scalar(device, input_a, scalar, dtype):
    torch_input_tensor_a = torch.as_tensor(input_a, dtype=dtype)
    torch_output_tensor = scalar * torch_input_tensor_a
    input_tensor_a = ttnn.from_torch(torch_input_tensor_a, layout=ttnn.TILE_LAYOUT, device=device)
    output = scalar * input_tensor_a
    output = ttnn.to_torch(output)
