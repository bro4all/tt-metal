...
def test_max_dim(device, input_shape, dim, dtype, keepdim):
    if is_grayskull() and (
        input_shape[-1] % 32 != 0 or input_shape[-2] % 32 != 0 or input_shape[dim] % 32 != 0 or dim <= -2
    ):
        pytest.skip("May fail on GS if run all the tests in this file. #17084")

    torch_input_tensor = torch_random(input_shape, -100, 100, dtype=dtype)
    torch_output_tensor, _ = torch.max(torch_input_tensor, dim=dim, keepdim=keepdim)
