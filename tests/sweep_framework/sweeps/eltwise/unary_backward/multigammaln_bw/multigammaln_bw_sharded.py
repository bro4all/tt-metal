...
model_traced_params = loader.get_suite_parameters("multigammaln_bw_sharded")
# To run all combinations: loader.get_suite_parameters("multigammaln_bw_sharded", all_cases=True)
...
def run(
    ...
    *,
    ...
    torch_input_tensor_a = gen_func_with_cast_tt(
        partial(torch_random, low=2.5 + 1e-6, high=100, dtype=torch.float32), input_a_dtype
    )(input_shape)
    ...
    result = ttnn.multigammaln_bw(grad_tensor, input_tensor_a, memory_config=sharded_config)[0]
