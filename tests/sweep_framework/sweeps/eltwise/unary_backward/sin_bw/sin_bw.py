...
model_traced_params = loader.get_suite_parameters("sin_bw")
# To run all combinations: loader.get_suite_parameters("sin_bw", all_cases=True)
...
def run(
    ...
    *,
    ...
    torch_grad_tensor = gen_func_with_cast_tt(
        partial(torch_random, low=0, high=6.283185307179586, dtype=torch.float32), grad_dtype
    )(input_shape)
    ...
    output_tensor = ttnn.sin_bw(grad_tensor, input_tensor_a, memory_config=output_memory_config)[0]
