...
model_traced_params = loader.get_suite_parameters("multigammaln")
# To run all combinations: loader.get_suite_parameters("multigammaln", all_cases=True)
...
def invalidate_vector(test_vector) -> Tuple[bool, Optional[str]]:
    if test_vector["input_layout"] == ttnn.ROW_MAJOR_LAYOUT or test_vector["input_dtype"] == ttnn.bfloat8_b:
        return True, "Row major layout and ttnn.bfloat8_b are not supported"
    return False, None
...
def run(
    ...
    *,
    ...
    torch_input_tensor = gen_func_with_cast_tt(
        partial(torch_random, low=1.6, high=100, dtype=torch.float32), input_dtype
    )(input_shape)
    torch_output_tensor = torch.special.multigammaln(torch_input_tensor, 4)
    ...
    result = ttnn.multigammaln(input_tensor, memory_config=output_memory_config)
