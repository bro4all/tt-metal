...
parameters = {
    "nightly": {
        ...
        "dtype": [ttnn.bfloat16, ttnn.bfloat8_b, ttnn.int32],
...
@pytest.mark.parametrize("shard_specs", parameters["nightly"]["shard_specs"])
@pytest.mark.parametrize("strategy", parameters["nightly"]["strategy"])
@pytest.mark.parametrize("orientation", parameters["nightly"]["orientation"])
@pytest.mark.parametrize("core_grid", parameters["nightly"]["core_grid"])
@pytest.mark.parametrize("dtype", parameters["nightly"]["dtype"])
@pytest.mark.parametrize("layout", parameters["nightly"]["layout"])
@pytest.mark.parametrize("input_buffer_type", parameters["nightly"]["input_buffer_type"])
@pytest.mark.parametrize("output_buffer_type", parameters["nightly"]["output_buffer_type"])
def test_run(
    shard_specs,
    strategy,
    orientation,
    ...
    *,
    ...
    skip, reason = invalidate_vector(test_vector)
    if skip:
        pytest.skip(f"Skipping test due to: {reason}")
    shape = shard_specs["shape"]
    shard_shape = shard_specs["shard_shape"]
