...
inline std::vector<Tensor> extract_output_tensors(const std::tuple<ttnn::Tensor, ttnn::Tensor, ttnn::Tensor>& t) {
...
std::vector<Tensor> extract_output_tensors(const std::variant<
                                           ttnn::Tensor,
...
auto query_op_constraints(Op op, tt::tt_metal::distributed::MeshDevice* device, Args&&... args) {
    ...
    // outer graph capture is to avoid dispatching/allocating dummy input tensors
    {
        ...
        try {
            ...
            return ConstraintQueryResponse{
                ExecutionStatus::Error, {0, 0, 0}, /* output_tensor_specs= */ std::nullopt, e.what()};
        }
    ...
    size_t l1_output_buffer_per_core =
        outputs[0].buffer()->is_dram()
            ? 0
            : extract_l1_output_buffer_allocation_size_per_core(outputs[0], interleaved_storage_cores);
