...
FORCE_INLINE uint32_t get_local_cb_tile_wr_ptr(uint32_t cb_id) {
    LocalCBInterface& local_cb = get_local_cb_interface(cb_id);
    return local_cb.fifo_wr_tile_ptr;
}
...
inline void tilize_one_subblock(
    uint32_t in_cb_id,
    ...
    uint32_t counter,
    bool reserve_back = true) {
    fast_tilize_init_with_dt(in_cb_id, in_block_w, out_cb_id);
    // TODO(sjovic): move out these divisions somehow
    int32_t patched_counter = (int32_t)counter - (int32_t)reuse_loops_start;
    if (patched_counter >= (int32_t)image_width_in_tiles && patched_counter % (int32_t)image_width_in_tiles == 0) {
        ...
        // UNPACK (( DPRINT << "new_read_ptr: " << new_read_ptr << ENDL() ));
    } else if (counter == reuse_loops_start) {
        UNPACK((update_local_cb_rd_ptr(in_cb_id, start_cb_addr)));
        // UNPACK (( DPRINT << "new_read_ptr: " << start_cb_addr << ENDL() ));
    } else {
...
inline void tilize_in_split_reader(
    uint32_t in1_cb_id,
    uint32_t in2_cb_id,
    uint32_t in_subblock_h,
    uint32_t in_block_w,
    ...
    uint32_t split_reader_diff,
    uint32_t start_cb_addr1,
    uint32_t start_cb_addr2,
    uint32_t image_width_in_tiles,
    uint32_t reuse_loops_start1,
    uint32_t reuse_loops_start2) {
    uint32_t counter = 0;
    uint32_t out_cb_tiles = in_block_w * in_subblock_h * (in1_num_subblocks + in2_num_subblocks);
    cb_reserve_back(out_cb_id, out_cb_tiles);

    uint32_t next_out_cb_addr1, next_out_cb_addr2;
    PACK((next_out_cb_addr1 = get_local_cb_interface(out_cb_id).fifo_wr_ptr));
    PACK((next_out_cb_addr2 = next_out_cb_addr1 + split_reader_diff));
    ...
    for (uint32_t in_subblock = 0; in_subblock < num_subblocks; ++in_subblock) {
        for (uint32_t h = 0; h < in_subblock_h; ++h) {
            if (in_subblock < in1_num_subblocks) {
                ...
                PACK((update_local_cb_wr_ptr(out_cb_id, next_out_cb_addr1)));
                PACK((update_local_cb_tile_wr_ptr(out_cb_id, 0)));
                PACK((next_out_cb_addr1 += element_diff));
                tilize_one_subblock(
                    in1_cb_id,
                    in_subblock_h,
                    in_block_w,
                    ...
                    reuse_loops_start1,
                    counter,
                    false  // reserve_back=false
                );
            }

            if (in_subblock < in2_num_subblocks) {
                PACK((update_local_cb_wr_ptr(out_cb_id, next_out_cb_addr2)));
                PACK((update_local_cb_tile_wr_ptr(out_cb_id, 0)));
                PACK((next_out_cb_addr2 += element_diff));
                tilize_one_subblock(
                    in2_cb_id,
                    in_subblock_h,
                    in_block_w,
...
void MAIN {
    ...
    uint32_t diff = get_compile_time_arg_val(28);
    uint32_t image_width_in_tiles = get_compile_time_arg_val(29);
    const uint32_t reuse_loops_start_1 = get_arg_val<uint32_t>(0);
    const uint32_t reuse_loops_start_2 = get_arg_val<uint32_t>(1);
...
#endif
                        // block or w sharded so we pass in zeros, never called
                        tilize_in(
                            in0_pretilize_cb_id,
                            in0_subblock_h,
                            in0_block_w,
