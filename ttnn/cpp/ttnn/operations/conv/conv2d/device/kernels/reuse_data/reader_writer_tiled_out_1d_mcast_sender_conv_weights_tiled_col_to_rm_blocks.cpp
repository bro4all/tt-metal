...
void kernel_main() {
    ...
    uint32_t weights_size_h, act_cb_tiles, w_tiles;
    if constexpr (split_reader) {
        weights_size_h = get_compile_time_arg_val(27);
        act_cb_tiles = get_compile_time_arg_val(28);
        w_tiles = get_compile_time_arg_val(29);
    }
    ...
    if constexpr (split_reader) {
        reuse_loops = get_arg_val<uint32_t>(i++);
        reuse_loops_start = get_arg_val<uint32_t>(i++);
    }
    ...
    *(weights_mcast_receiver_semaphore_addr_ptr) = VALID;
    ...
    uint32_t start_reader_idx, cb_start_addr;
    if constexpr (split_reader) {
        reader_idx = (uint32_t)(packed_reader_indices_ptr[0] & 0xffff) + 1;
    }
        ...
        for (uint32_t block_weight_h = 0; block_weight_h < num_blocks_weight_h; block_weight_h++) {
            if constexpr (split_reader) {
                for (uint32_t loop = 0; loop < reuse_loops; loop++) {
                    cb_reserve_back(cb_id_act_second_reader, act_cb_tiles);
                    if (loop <= reuse_loops_start) {
                        if (bh == 0 && loop == 0) {
                            cb_start_addr = get_write_ptr(cb_id_act_second_reader);
                        }
                        l1_write_addr_act = cb_start_addr;
                    } else {
                        l1_write_addr_act =
                            cb_start_addr + (loop - reuse_loops_start) * weight_size_w * conv_act_c_read_bytes;
                        update_local_cb_wr_ptr(cb_id_act_second_reader, l1_write_addr_act);
                    }
...
#ifndef SKIP_MCAST
                DPRINT << "weights_mcast_num_dests " << weights_mcast_num_dests << ENDL();
