...
void GroupNormV3::validate(
    ...
    const auto& device = src_tensor.device();
    ...
    const auto& num_required_cores = this->core_grid.x * this->core_grid.y;
    TT_FATAL(
...
std::vector<TensorSpec> GroupNormV3::compute_output_specs(const std::vector<Tensor>& input_tensors) const {
    if (this->inplace) {
    ...
    return {TensorSpec(
        input_tensor.logical_shape(),
        TensorLayout(this->output_dtype, PageConfig(input_tensor.layout()), this->output_mem_config))};
}

std::vector<Tensor> GroupNormV3::create_output_tensors(const std::vector<Tensor>& input_tensors) const {
    if (this->inplace) {
        return {input_tensors.at(0)};
    }
    return {create_device_tensor(this->compute_output_specs(input_tensors).at(0), input_tensors.at(0).device())};
}

operation::ProgramWithCallbacks GroupNormV3::create_program(
    const std::vector<Tensor>& input_tensors,
    const std::vector<std::optional<const Tensor>>& optional_input_tensors,
    std::vector<Tensor>& output_tensors) const {
    const auto& a = input_tensors.at(0);
