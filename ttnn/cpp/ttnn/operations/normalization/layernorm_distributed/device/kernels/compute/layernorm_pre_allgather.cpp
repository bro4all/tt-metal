...
void MAIN {
    ...
    DPRINT << "LAYERNORM_PRE_ALLGATHER: NCHt=" << NCHt << " Wt=" << Wt << " blk=" << blk
           << " is_merge_core=" << (int)is_merge_core << ENDL();
    ...
    DPRINT << "LAYERNORM_PRE_ALLGATHER: Got cb_reduce" << ENDL();
    ...
    for (uint32_t ncht = 0; ncht < NCHt; ncht++) {
        ...
        for (uint32_t wt = 0; wt < Wt; wt += blk) {
            ...
            DPRINT << "LAYERNORM_PRE_ALLGATHER: Got cb_inp" << ENDL();
            ...
            DPRINT << "LAYERNORM_PRE_ALLGATHER: Pushing cb_x2" << ENDL();
         ...
         * sum(x**2)
         */
        DPRINT << "LAYERNORM_PRE_ALLGATHER: Starting sum(x**2) computation" << ENDL();
        reconfig_data_format(cb_x2, cb_reduce);
        pack_reconfig_data_format(cb_out);
        reduce_init(cb_x2, cb_reduce, cb_out);

        cb_wait_front(cb_x2, Wt);
        DPRINT << "LAYERNORM_PRE_ALLGATHER: Got cb_x2" << ENDL();
    ...
    if (is_merge_core) {
        ...
        constexpr uint32_t cb_out_final = tt::CBIndex::c_16;
        constexpr int onetile = 1;
        constexpr int dst0 = 0;
        ...
        add_tiles_init(cb_x2_merge, cb_out_final, true);
        ...
        for (uint32_t i = 0; i < num_cores_y; i++) {
            add_tiles(cb_x2_merge, cb_out_final, i, 0, dst0);
        }
    ...
    DPRINT << "LAYERNORM_PRE_ALLGATHER: Kernel complete" << ENDL();
}
}  // namespace NAMESPACE
