...
void ControlPlane::initialize_dynamic_routing_plane_counts(
    ...
    for (auto mesh_id : user_meshes) {
        ...
        if (may_have_intra_mesh_connectivity) {
            ...
            auto global_rows_min = *std::min_element(row_min_planes.begin(), row_min_planes.end());
            auto global_cols_min = *std::min_element(col_min_planes.begin(), col_min_planes.end());
            ...
            if (*distributed_context.size() > 1) {
                ...
                distributed_context.all_gather(
                    tt::stl::Span<std::byte>(reinterpret_cast<std::byte*>(&global_rows_min), sizeof(size_t)),
                    tt::stl::as_writable_bytes(tt::stl::Span<size_t>{rows_min_buf.data(), rows_min_buf.size()}));
                distributed_context.all_gather(
                    tt::stl::Span<std::byte>(reinterpret_cast<std::byte*>(&global_cols_min), sizeof(size_t)),
                    tt::stl::as_writable_bytes(tt::stl::Span<size_t>{cols_min_buf.data(), cols_min_buf.size()}));
                distributed_context.barrier();
                global_rows_min = *std::min_element(rows_min_buf.begin(), rows_min_buf.end());
                global_cols_min = *std::min_element(cols_min_buf.begin(), cols_min_buf.end());
            }
