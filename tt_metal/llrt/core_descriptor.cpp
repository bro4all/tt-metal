...
inline std::string get_core_descriptor_file(
    ...
    if (targeting_sim) {
        ...
        std::string blackhole_variant = dispatch_core_config.get_core_type() == CoreType::ETH
        ...
        if (getenv("TT_METAL_BLACKHOLE_CORE_VARIANT")) {
            ...
            if (!std::filesystem::exists(core_desc_dir + blackhole_variant_a)) {
                TT_THROW(
                    "Blackhole core descriptor variant {} does not exist in {}", blackhole_variant_a, core_desc_dir);
            }
        ...
        switch (arch) {
            ...
            case tt::ARCH::BLACKHOLE: return core_desc_dir + blackhole_variant;
            case tt::ARCH::QUASAR: TT_THROW("No core descriptor for Quasar"); break;
        }
    }
