...
import ttnn
from models.demos.qwen3_moe.reference.modeling_qwen3_moe import Qwen3MoeExperts

# Import from local reference files instead of HuggingFace
from models.demos.qwen3_moe.tt.expert import Expert
...
def reference_model(hf_config):
    """Get the actual Qwen3Moe MLP model using local implementation."""
    return Qwen3MoeExperts(hf_config)
...
def test_forward_pass(
    ...
    weight_config = Expert.convert_weights_moe(hf_config_single_layer, hf_state_dict, temp_dir, galaxy_or_t3k_mesh)
    # Generate appropriate config
    if mode == "prefill":
        model_config = Expert.prefill_model_config(hf_config_single_layer, galaxy_or_t3k_mesh)
    else:
        model_config = Expert.decode_model_config(hf_config_single_layer, galaxy_or_t3k_mesh)

    # Create a new model state
    model_state = Expert.create_state(hf_config_single_layer, mesh_device=galaxy_or_t3k_mesh)
    ...
    if mode == "prefill":
        ...
        tt_output = Expert.forward_prefill(tt_input, run_config)
        expected_output_memory_config = run_config["output_memory_config"]
    else:
        tt_input = ttnn.to_memory_config(tt_input, run_config["input_memory_config"])
        tt_output = Expert.forward_decode(tt_input, run_config)
        expected_output_memory_config = run_config["output_memory_config"]
    ...
    assert passing, f"Expert output does not meet PCC requirement {pcc_required}: {pcc_message}"
