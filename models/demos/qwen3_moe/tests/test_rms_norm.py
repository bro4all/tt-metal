...
import ttnn
from models.demos.qwen3_moe.reference.modeling_qwen3_moe import Qwen3MoeRMSNorm
from models.demos.qwen3_moe.tt.rms_norm import RMSNorm
from models.demos.qwen3_moe.utils.config_helpers import NORM_CATEGORIES
...
def hf_config():
    ...
    path = os.getenv("HF_MODEL", "/home/share/Qwen3-235B-A22B")
...
def reference_model(hf_config, request):
    """Get the actual Qwen3MoE RMSNorm model using local implementation."""
    ...
    logger.info(f"Creating reference model with hidden_size: {hidden_size}, norm_category: {norm_category}")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("decode", 32, 1, "q_norm"),  # Q norm test (uses head_dim)
        ("prefill", 1, 128, "q_norm"),  # Q norm prefill test
        ("decode", 32, 1, "k_norm"),  # K norm test (uses head_dim)
...
def test_rmsnorm_forward_pass(
    ...
    logger.info(f"Reference model state dict keys: {list(hf_state_dict.keys())}")
    ...
    if is_decoder_norm and mode == "decode":
        shard_core_grid = ttnn.CoreGrid(x=4, y=4)
    ...
    logger.info(f"TTNN input shape: {tt_input.shape}, dtype: {tt_input.dtype}")
    # TTNN forward pass
    if mode == "decode":
    ...
    logger.info(f"TTNN output shape: {tt_output_torch.shape}, dtype: {tt_output_torch.dtype}")
