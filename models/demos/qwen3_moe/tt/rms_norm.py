...
from models.demos.qwen3_moe.utils.config_dataclass import FromWeightConfig, MeshDeviceStub, RMSNormConfig
from models.demos.qwen3_moe.utils.config_helpers import COMPUTE_KERNEL_CONFIG_HIFI2, NORM_CATEGORIES, save_and_get_path
from models.demos.qwen3_moe.utils.run_config import ModelDecodeConfig, ModelPrefillConfig, WeightConfig
...
class RMSNorm(AbstractModule):
    ...
    def convert_weights(
        ...
        logger.info(f"Converting RMSNorm weights for norm category: {norm_category}, shape: {torch_weight.shape}")
        ...
        logger.info(f"Converted RMSNorm weight shape: {torch_weight.shape}")
    ...
    def _distributed_rmsnorm(
        ...
        logger.info(f"RMSNorm input shape: {inp.shape}, dtype: {inp.dtype}")
        # Run distributed rmsnorm part 1
        tt_stats = ttnn.rms_norm_pre_all_gather(inp, program_config=program_config, dtype=output_dtype)

        logger.info(f"RMSNorm stats shape: {tt_stats.shape}, dtype: {tt_stats.dtype}")
        ...
        logger.info(f"RMSNorm part 2 inp shape: {inp.shape}")
        logger.info(f"RMSNorm part 2 stats shape: {tt_stats.shape}")
        logger.info(f"RMSNorm part 2 weight shape: {weight.shape}")
