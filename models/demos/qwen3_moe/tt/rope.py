...
import ttnn
from models.demos.qwen3_moe.reference.modeling_qwen3_moe import Qwen3MoeRotaryEmbedding
...
def get_cos_sin_matrix(hf_config):
    ...
    """
    reference_rope = Qwen3MoeRotaryEmbedding(hf_config)

    dummy_x = torch.zeros(1, 1, hf_config.head_dim, dtype=torch.bfloat16)  # shape: [batch, seq_len, dim]
    position_ids = torch.arange(hf_config.max_seq_len).unsqueeze(0)  # [1, max_seq_len]
    ...
    cos = cos.unsqueeze(0)  # [1, 1, max_seq_len, dim]
    sin = sin.unsqueeze(0)  # [1, 1, max_seq_len, dim]
...
class RotarySetup:
    """
    Class to set up rotary positional embeddings for Qwen3Moe models.
