...
def reference_forward(
    ...
    if mode == "decode":
        ...
        # Pad the input to largest position idx
        max_position_idx = position_ids.max().item()
        new_input_tensor = torch.zeros(bsz, max_position_idx + 1, dim, dtype=torch_input.dtype)

        for i, pos_idx in enumerate(position_ids):
            new_input_tensor[i, pos_idx] = torch_input[i, 0]
